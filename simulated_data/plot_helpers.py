...
def plot_quality_map(value,position,name="",title="", mask = None):
    """AKA world map
    ...
    fig = plt.figure(name,figsize=(20,10))#figsize=(13,9))#
    #plt.tight_layout()
    #plt.title(title)
    ...
    #norm = matplotlib.colors.LogNorm(levels)
...
def plot_nice_world(value,position,name="Satelite positon",title="", mask = None):
    ...
    world.plot(ax=ax, color='', edgecolor='black')
...
def save_results():
    ...
    for i, fig in enumerate(figs):
        fig.savefig(image_path.joinpath(f"{labels[i]}.png"),format='png')
